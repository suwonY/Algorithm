...
public class Baek1102_fix2 {
	...
	public static int go(int cnt, int now){
		...
		for(int i=0; i<n; i++){
			int temp = 1<<i;
//			System.out.println("i: " + i +"일 때" + " 현재 now: " + now + " 확인후 now : " + (now & temp));
...
//				System.out.println(i+"로 "+j+ " 를 켰을 때 :"+(now+temp));
...
//		System.out.println("맨 처음 now : " + now);
		int ans = go(cnt,now);
