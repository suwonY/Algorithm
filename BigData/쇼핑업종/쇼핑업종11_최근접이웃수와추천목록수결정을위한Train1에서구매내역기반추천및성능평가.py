...
id_train1_current = pd.read_csv('C:/Users/조만재/Desktop/lpoint/lpoint/date_rating_train1.csv', sep=',', dtype={'ID':str} ,engine='python')
id_train2_current = pd.read_csv('C:/Users/조만재/Desktop/lpoint/lpoint/date_rating_train2.csv', sep=',', dtype={'ID':str} ,engine='python')
id_train3_current = pd.read_csv('C:/Users/조만재/Desktop/lpoint/lpoint/date_rating_train3.csv', sep=',', dtype={'ID':str} ,engine='python')
id_train4_current = pd.read_csv('C:/Users/조만재/Desktop/lpoint/lpoint/date_rating_train4.csv', sep=',', dtype={'ID':str} ,engine='python')
id_train5_current = pd.read_csv('C:/Users/조만재/Desktop/lpoint/lpoint/date_rating_train5.csv', sep=',', dtype={'ID':str} ,engine='python')


#9월 30일 이후
id_train_future = pd.read_csv('C:/Users/조만재/Desktop/lpoint/lpoint/date_test.csv', sep=',', dtype={'ID':str} ,engine='python')

P_u1_current=id_train1_current
P_u4_current=pd.concat((id_train2_current, id_train3_current, id_train4_current, id_train5_current))
...
#Top_cnt와 Recommend의 갯수를 변화시켜가며 코드 실행
...
Ruv=1-sklearn.metrics.pairwise.pairwise_distances(X=P_u1_current, Y=P_u4_current, metric='cosine')
Ruv=pd.DataFrame(Ruv)
Ruv.index=P_u1_current.index
Ruv.columns=P_u4_current.index
...
for i in range(len(result_item)):
    h1 = list(result_item.ix[i])
    hindex =result_item.index[i]
    hdic = makeDic(col_list_test,id_train_future.ix[hindex])
    ht = T(hdic)
    for i in h1:
        if i in ht:
            hcnt += 1
            hsum += 1                     
    try: hit_ratio.append(hcnt/hsum)        
    except: hit_ratio.append(0)
    hcnt=0
    hsum=0
...
eval.append(np.sum(lift_ratio)/1600)    
...
#Diversity
id_train_current = pd.read_csv('C:/Users/조만재/Desktop/lpoint/lpoint/date_train.csv', sep=',', dtype={'ID':str} ,engine='python')
...
indexlist = list(result_item.index)
for i in indexlist:
    aaa = list(result_item.ix[i])
    percnt = 0
    for j in indexlist:
        if j != i:
            bbb = list(result_item.ix[j])
            for i in aaa:
                if i not in bbb:
                    percnt += 1
            personality.append(percnt/len(aaa))
            percnt = 0
    personality1.append(np.mean(personality))
    personality = []
np.mean(personality1)
eval.append(np.mean(personality1))
