...
import java.util.*;
public class swExpert2477_fix3 {
...
//								System.out.println(i+"번째 손님  " + j+"번 접수창구로 이동");
				...
				//정비 대기자들 중에서 접수창구 제일 낮은 순서부터 너야됨
				while(true){
					int min = 987654321;
					int next = -1;
					for(int i=1; i<=k; i++)
						if(customer[i].situation==3)
							if(customer[i].anum<min)
								next = i;
						
					if(next==-1)break;
					boolean finish = true;
					for(int i=1; i<=m; i++){//빈 접수창구 확인
						if(c2[i]==0){//빈 정비창구가있으면
//							System.out.println(i+"번째 손님  " + j+"번 정비창구로 이동");
							customer[next].situation=4;//정비중 상태로 바꾸고
							customer[next].bnum=i;//고객의 정비창구번호 갱신
							c2[i]=b[i];//정비시간 갱신
							finish = false;
							break;
						}
