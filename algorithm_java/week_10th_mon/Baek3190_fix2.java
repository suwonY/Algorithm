...
import java.util.*;
public class Baek3190_fix2 {

	static int n, k, l, time=0, dir=4, hx=0, hy=0;
	static int when = -1;
	static char where = ' ';
	static Deque<Point> tails = new ArrayDeque<>();
	static Deque<Change> q = new ArrayDeque<>();
	static Change c = null;
	...
	public static void change(){
		...
		//시간이 됐으면 방향 바꿔주기
		if(time==when){
			if(dir==1){ //상일때 좌로 돌면 좌 우로돌면 우
				if(where=='L') dir=3;
				else dir=4;
			}
			else if(dir==2){ //하일때 좌로 돌면 우 우로돌면 좌
				if(where=='L') dir=4;
				else dir=3;
			}
			else if(dir==3){ //좌일때 좌로돌면 하 우로돌면 상
				if(where=='L') dir=2;
				else dir=1;
			}
			else{ //우일때 좌로돌면 상 우로돌면 하
				if(where=='L') dir=1;
				else dir=2;
			}
			c=null;
...
//		change(t[time]);
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<l; i++){
			x += in.nextInt();
			String temp = in.next();
			q.addLast(new Change(x,temp.charAt(0)));
		}
	...
	public static class Change{
		int time;
		char dir;
		public Change(int time, char dir){
			this.time = time;
			this.dir = dir;
		}
	}
